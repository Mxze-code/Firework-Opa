...
create table if not exists public.user_preferences (
  ...
  build_focus text,
  supporting_priorities text[] default '{}',
  selected_tracking_systems text[] default '{}',
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
...
-- Target completions
create table if not exists public.target_completions (
  id uuid default uuid_generate_v4() primary key,
  user_id uuid references public.profiles(id) on delete cascade not null,
...
-- Indexes
create index if not exists idx_target_completions_user_date on public.target_completions(user_id, completed_at desc);
...
drop policy if exists "player_stats_all" on public.player_stats;
create policy "player_stats_all" on public.player_stats for all using (auth.uid() = user_id);
drop policy if exists "daily_targets_all" on public.daily_targets;
create policy "daily_targets_all" on public.daily_targets for all using (auth.uid() = user_id);
drop policy if exists "target_completions_all" on public.target_completions;
...
drop policy if exists "recovery_logs_all" on public.recovery_logs;
create policy "recovery_logs_all" on public.recovery_logs for all using (auth.uid() = user_id);
drop policy if exists "xp_events_all" on public.xp_events;
...
drop policy if exists "personal_goals_all" on public.personal_goals;
